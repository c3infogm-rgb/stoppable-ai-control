...
通常のレビューでも、制御上の問題候補を見つけることはできます。弊会の内部Referenceでも、通常寄りの防御レビューから事前拒否とイベント処理の契約に関する候補地点へ到達しました。

制御条件ギャップチェックの差は、「通常では見つからない」と主張することではありません。見つけた候補を、Declared Control・Comparator・Effect Boundary・Variant・Replayへ分解し、**どの条件でControlが維持され、どこで結果が変わるかを再検証できる形にする**ことです。

→ **[通常レビューと制御条件ギャップチェックの比較を見る](control-condition-test-cases/NORMAL_REVIEW_VS_GAP_CHECK.md)**
...
公開6ケースは、検査体系全体ではなく、方法論を確認するための代表的な公開Referenceです。

弊会の非公開の制御条件ギャップチェックでは、13軸のVariant taxonomyを**検査設計の候補空間**として使用し、対象となるDeclared Control、実行経路、Effect Boundary、証拠条件に応じて適用する軸を選定します。必要に応じて、複数軸を組み合わせたReplayも検討対象になります。

これは「すべての案件で13軸を機械的に総当たりする」という意味ではありません。また、13軸を確認すれば安全性・完全性・脆弱性不存在が証明されるという意味でもありません。

公開しているのは、軸の体系と代表6ケースまでです。案件ごとの軸選定ロジック、組合せ戦略、優先順位付けの詳細は公開していません。
